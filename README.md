...
```bash
europe-exchange-rate$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
europe-exchange-rate$ sam logs -n HelloWorldFunction --stack-name europe-exchange-rate --tail
```
...
```bash
aws cloudformation delete-stack --stack-name europe-exchange-rate
```
